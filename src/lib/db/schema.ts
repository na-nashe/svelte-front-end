import { pgTable, foreignKey, serial, integer, varchar, boolean, text, unique, uuid, timestamp, check, smallint, primaryKey, pgEnum } from "drizzle-orm/pg-core"
...
export const users = pgTable("users", {
	...
	avatar: text(),
	joined: timestamp({ withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
}, (table) => [
...
export const productAlternatives = pgTable("product_alternatives", {
	...
	primaryKey({ columns: [table.productId, table.alternativeId], name: "product_alternatives_pkey"}),
]);

export const userSaved = pgTable("user_saved", {
	userId: uuid("user_id").notNull(),
	alternativeId: integer("alternative_id").notNull(),
	savedAt: timestamp("saved_at", { withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
}, (table) => [
	foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],
