...
<div class="animate-slide-r w-[200px] shrink-0 sticky top-[76px]" style="animation-delay: 0.15s">
	<p class="mb-2 px-2 font-[JetBrains_Mono] text-[10px] font-bold uppercase tracking-widest text-stone-400">
		Навігація
	</p>
	{#each CATS as c, i (c.slug)}
		...
		{#if count > 0}
			<button
				...
				class="mb-px flex w-full cursor-pointer items-center gap-2 rounded-[10px] border-none px-2.5 py-[9px] font-[Outfit] text-xs transition-all duration-200 animate-slide-r
					{active ? 'bg-[#0057B708] font-bold text-[#0057B7]' : 'bg-transparent font-medium text-stone-500 hover:bg-stone-50'}"
			...
			>
				...
				<span
					class="rounded-[5px] px-1.5 py-px font-[JetBrains_Mono] text-[10px]
						{active ? 'bg-[#0057B70a] text-[#0057B7]' : 'bg-stone-100 text-stone-300'}"
				>{count}</span>
			</button>
		{/if}
	{/each}
	...
	<div class="mt-5 rounded-2xl border border-stone-200 bg-white p-3.5 px-[14px]">
		...
		{#each insights as ins, i (ins.value)}
			<div class="flex items-center gap-2 py-[7px]" style="border-bottom: {i < insights.length - 1 ? '1px solid #f5f5f4' : 'none'}">
				<div
				...
				<span class="font-[JetBrains_Mono] text-[10px] font-bold" style="color: {ins.cl}">{ins.num}</span>
			</div>
		{/each}
	</div>
</div>
